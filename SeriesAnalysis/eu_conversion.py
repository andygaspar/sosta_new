...
def read_and_set_df_eu(year: int):
    ...
    t_min = df_eu["dep time"].apply(lambda t: np.round(t.hour * 60 + t.minute + t.second * 0.1)).astype(int)
    df_eu["dep_min"] = t_min
    t_min = df_eu["arr time"].apply(lambda t: np.round(t.hour * 60 + t.minute + t.second * 0.1)).astype(int)
    df_eu["arr_min"] = t_min
